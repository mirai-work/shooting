...
class VirtualController:
    ...
    def draw(self, is_game_over):
        if not is_game_over:
            ...
            pyxel.text(rx + 6, ry + 3, "TAP TO RESTART", 7)
...
def wrapped_update(self):
    ...
    _old_btn = pyxel.btn
    _old_btnp = pyxel.btnp # btnp(1回押し判定)もハック対象に追加
    ...
    # 1回押し判定(Restart用)のハック
    def custom_btnp(key):
        if is_touch:
            # ゲームオーバー中にリスタート範囲をタップしたらRキーをシミュレート
            if self.game_over and key == pyxel.KEY_R:
                if _v_controller._check(tx, ty, _v_controller.btn_restart): return True
            if key == pyxel.KEY_RETURN: return True
...
def wrapped_draw(self):
    _original_draw(self)
    # タイトル画面以外で仮想ボタンを表示
    if not getattr(self, 'show_title', True):
