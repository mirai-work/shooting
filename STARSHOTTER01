...
class StarSoldier:
    ...
    def init_sound(self):
        pyxel.sounds[0].set("c3c3c3", "n", "3", "f", 5) 
        pyxel.sounds[1].set("c1g1c1", "n", "4", "f", 10)
        pyxel.sounds[2].set("e3g3c4", "n", "3", "v", 20)
    ...
    def handle_start_button(self):
        if self.scene == SCENE_TITLE:
            pyxel.play(2, 2); self.reset_all(); self.scene = SCENE_PLAY
        elif self.scene == SCENE_PLAY: self.scene = SCENE_PAUSE
        elif self.scene == SCENE_PAUSE: self.scene = SCENE_PLAY
        elif self.scene == SCENE_ENDING: self.scene = SCENE_TITLE

    def update_play_logic(self):
        self.frame += 1
        # タイトル画面なら強制的に1面のアルゴリズムを使用
    ...
    def hit_player(self):
        ...
        if self.lives <= 0: self.scene = SCENE_GAMEOVER; self.gameover_timer = 0
    ...
    def update_boss(self):
        ...
        # ボス弾幕（ステージ別）
        if self.stage == 1:
            if pyxel.frame_count % 25 == 0: self.enemy_shots.append([b["x"], b["y"]])
        elif self.stage == 2:
            if pyxel.frame_count % 20 == 0:
                for angle in [-10, 0, 10]: self.enemy_shots.append([b["x"] + angle, b["y"]])
        elif self.stage == 3:
            if pyxel.frame_count % 18 == 0:
                for angle in [-20, -10, 0, 10, 20]: self.enemy_shots.append([b["x"] + angle, b["y"]])
        elif self.stage == 4:
            if pyxel.frame_count % 15 == 0:
                for angle in [-30, -15, 0, 15, 30]: self.enemy_shots.append([b["x"] + angle, b["y"]])
        elif self.stage == 5:
            if pyxel.frame_count % 10 == 0:
                for angle in [-40, -20, -10, 0, 10, 20, 40]: self.enemy_shots.append([b["x"] + angle, b["y"]])
        ...
        for s in self.shots[:]:
            if abs(s[0]-b["x"]) < 20 and abs(s[1]-b["y"]) < 15:
                ...
                self.score += 50
        if b["hp"] <= 0:
            self.score += 10000; self.scene = SCENE_STAGE_CLEAR; self.boss = None; pyxel.play(2, 2)

    def next_stage(self):
        if self.stage >= MAX_STAGE: self.scene = SCENE_ENDING
        else:
