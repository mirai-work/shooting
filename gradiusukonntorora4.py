...
class Player:
    ...
    def update(self):
        ...
        # --- 1. キーボード・パッド入力 ---
        if pyxel.btn(pyxel.KEY_UP) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_UP): dy = -2
        if pyxel.btn(pyxel.KEY_DOWN) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_DOWN): dy = 2
        if pyxel.btn(pyxel.KEY_LEFT) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_LEFT): dx = -2
        if pyxel.btn(pyxel.KEY_RIGHT) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_RIGHT): dx = 2
        if pyxel.btn(pyxel.KEY_SPACE) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_A): is_shooting = True

        # --- 2. バーチャルパッド入力 (画面下部の y > 120 エリア) ---
        if pyxel.btn(pyxel.MOUSE_BUTTON_LEFT):
            ...
            if my > SCREEN_H:
                ...
                if mx < 80:
                    ...
                    if my < SCREEN_H + 15: dy = -2
                    elif my > SCREEN_H + 25: dy = 2
                # 右側：ショット
                else:
                    is_shooting = True
        ...
        self.shot_timer += 1
        if is_shooting and self.shot_timer % 5 == 0:
            if self.power == 1:
...
class App:
    def __init__(self):
        ...
        # 【修正】タイトルとインターリュードをスキップして即開始する設定
        self.show_title = False     # タイトル画面を表示しない
        self.stage_interlude = False # インターリュード（説明画面）を表示しない
        self.stage = 0              # ステージ1 (内部数値0) から開始
        self.enemies_to_spawn = 10  # ステージ1の敵出現数
    ...
    def reset(self, is_initial_start=False):
        ...
        self.powerups = []
        self.boss = None
        self.stage = 0 # 【修正】ここも0（ステージ1）からにする
        self.stage_timer = 0
        self.score = 0
    ...
    def update_interlude(self):
        ...
        if self.is_tutorial_interlude:
            ...
            if (pyxel.btnp(pyxel.KEY_RETURN) or pyxel.btnp(pyxel.GAMEPAD1_BUTTON_B)) and self.interlude_timer > 30:
                self.end_interlude()
            return
    ...
    def update(self):
        ...
        # GAME OVER
        if self.game_over:
            if pyxel.btnp(pyxel.KEY_RETURN) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_B):
                self.reset(is_initial_start=False)
    ...
    # -------------------
        ...
        if self.game_over:
            ...
            pyxel.text(45, 80, "RETURN/B KEY!", 10)
